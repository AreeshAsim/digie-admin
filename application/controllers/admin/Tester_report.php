...
class Tester_report extends CI_Controller {
    ...
    //     $volume_check = false;
    //     $vol_percentile_chk = false;
    //     $neg_vol_percentile_chk = false;
    //     $neg_clr_chk = true;
    ...
    //     if (empty($data_arr['total_volume_percentile']) || ($result['total_volume_percentile'] <= $data_arr['total_volume_percentile'] && $result['total_volume_percentile'] > 0)) {
    //         $vol_percentile_chk = true;

    //     }
    //     if ($data_arr['negate_vol_chk'] == 'yes') {
    //         if (empty($data_arr['total_volume_percentile_negate']) || ($result['total_volume_percentile'] <= $data_arr['total_volume_percentile_negate'] && $result['total_volume_percentile'] > 0)) {
    //             $neg_vol_percentile_chk = true;
    ...
    //     if ($last_candle_buy && $last_candle_sell && $wick_filter && $last_demand_candle && $status && $swing && $type && $move && $condition && $big && $big_h && $condition_h && $big_contractors && $move_clr && $big_buy && $big_sell && $rejection && $total_vol && $volume_check && $vol_percentile_chk && $neg_vol_percentile_chk && $neg_clr_chk) {
    //         $new = true;
    ...
    //         $total_profit2 += $value['total_profit2'];
    //         $winning2 += $value['winning2'];
    //         $losing2 += $value['losing2'];
    //         $winners2 = $value['winners2'];
    //         $losers2 += $value['losers2'];
    ...
    //     $total_trades2 = $winning2 + $losing2;
    //     $win_per2 = ($winning2 / $total_trades2) * 100;
    //     $lose_per2 = ($losing2 / $total_trades2) * 100;

    //     $total_per_trade2 = $total_profit2 / $total_trades2;
    //     $total_per_day2 = $total_profit2 / $total_number_of_days;
    ...
    //     $final['losers'] = $losers;
    //     $final['winning2'] = $winning2;
    //     $final['losing2'] = $losing2;
    //     $final['winners2'] = $winners2;
    //     $final['losers2'] = $losers2;
    //     $final['symbol'] = $symbol;
    ...
    //     $final['per_day2'] = $total_per_day2;
    //     $final['win_per2'] = $win_per2;
    //     $final['lose_per2'] = $lose_per2;
    //     $final['per_trade2'] = $total_per_trade2;
    ...
    // public function check_increased_volume_testing($hour = "2019-08-06 21:00:00", $symbol = "NEOBTC") {
    ...
    //     echo $new_volume = $res[0]['total_volume'];
    //     echo "-=========->";
    //     echo $old_volume = $res[1]['total_volume'];

    //     echo "------------->";
    //     echo $new_candle_color = (($res[0]['close'] > $res[0]['open']) ? "blue" : "red");
    //     if (($new_volume > $old_volume) && $new_candle_color == "blue") {
    //         echo "True Condition";
    //     } else {
    ...
    // public function step_two_test($symbol = "QTUMBTC", $start_date = "2019-08-27 09:00:00", $end_date = "2019-08-27 10:00:00") {
    //     $start = date("Y-m-d H:00:00", strtotime("-1 hour", strtotime($start_date)));
    //     $end = date("Y-m-d H:00:00", strtotime("-1 hour", strtotime($end_date)));
    //     $candle = $this->get_candle($symbol, $start);
    //     echo "<pre>";
    //     print_r($candle);
    //     $high = $candle['high'];
    ...
    // public function top_wick_aggregate_simulator($symbol = "XRPUSDT", $start = "2019-07-28 10:00:00", $end = "2019-07-28 11:00:00") {
    //     //$start = date("Y-m-d H:00:00", strtotime("-1 hour", strtotime($start_date)));
    //     //$end = date("Y-m-d H:00:00", strtotime("-1 hour", strtotime($end_date)));
    //     $candle = $this->get_candle($symbol, $start);
    //     $high = $candle['high'];
    ...
    //     for ($i = 0; $i < 2; $i++) {
    //         $price = $data[$i]['price'];
    //         if ($price <= (float) $high && $price > (float) $close) {
    //             $top = true;
    //             $price_arr[] = num($price);
    //             break;
    //         } else {
    ...
    //     }
    //     if ($top == true) {
    //         $test = array("top_wick_aggregate" => false, "top_wick_prices" => implode(", ", $price_arr));
    //     } else {
    //         $test = array("top_wick_aggregate" => true, "top_wick_prices" => implode(", ", $price_arr));
    //     }

    //     $this->mongo_db->where(array("_id" => $candle_id));
    //     $this->mongo_db->set($test);
    //     $this->mongo_db->update("market_chart");
    // }

    // public function testing_percentiles($symbol, $value = '') {
    //     //echo urldecode($value);
    //     //echo "<br>";
    //     $date = date("Y-m-d 00:00:00", strtotime(urldecode($value)));

    //     $this->mongo_db->where(array('coin' => $symbol, "modified_date" => array('$lte' => $this->mongo_db->converToMongodttime($date))));
    //     $this->mongo_db->limit(30);
    //     $this->mongo_db->order_by(array("modified_date" => -1));
    //     $get = $this->mongo_db->get("market_trade_daily_percentile");
    ...
    //     $return_arr['buyers']['top10'] = $percentile10;
    //     $return_arr['buyers']['top5'] = $percentile5;
    //     $return_arr['buyers']['top4'] = $percentile4;
    //     $return_arr['buyers']['top3'] = $percentile3;
    //     $return_arr['buyers']['top2'] = $percentile2;
    //     $return_arr['buyers']['top1'] = $percentile1;
    ...
    //     $return_arr['sellers']['top10'] = $percentile10;
    //     $return_arr['sellers']['top5'] = $percentile5;
    //     $return_arr['sellers']['top4'] = $percentile4;
    //     $return_arr['sellers']['top3'] = $percentile3;
    //     $return_arr['sellers']['top2'] = $percentile2;
    //     $return_arr['sellers']['top1'] = $percentile1;
