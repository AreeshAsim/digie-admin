...
class Buy_orders extends CI_Controller
{
    ...
    public function update_now_process(){
        ...
        $set_arr = [
            ...
            'binance_order_id_sell' =>  $post_data['binance_order_id_sell'],
    ...
    public function get_order_ajax(){
        ...
        if (isset($id) && isset($userid) && isset($exchange)) {
        ...
        */
        ...
        foreach ($sell_order as $attr => $value) {
            $in_arr = ['order_level','auto_sell','purchased_price', 'quantity', 'symbol', 'stop_loss', 'trigger_type', 'sell_price', 'iniatial_trail_stop', 'loss_percentage', 'sell_profit_percent', 'market_value', 'sell_binance_order_id', 'market_value_usd', 'status', 'trail_check', 'trail_interval', 'sell_trail_price'];
            if (in_array($attr, $in_arr)) {
                if (gettype($value) == 'string') {
                    if ($attr != 'status' && $attr != 'trigger_type' && $attr != 'symbol') {
                        $html1 .= '
    ...
    public function get_order_by_id($id, $userid, $exchange){
        $this->mod_login->verify_is_admin_login();
        // ini_set("display_errors", E_ALL);
        // $exchange == 'bam';
        error_reporting(E_ALL);
    ...
    public function sell_array(){
        ...
        if(isset($exchange) &&  isset($account_id) && isset($order_id)){
            $buy_array = $this->get_buy_array($exchange, $account_id, $order_id);
        } 
        $date = date('Y-m-d G:i:s');
        if(isset($buy_array)){
    ...
    public function get_buy_array($exchange, $account_id, $order_id){
        $this->mod_login->verify_is_admin_login();
        if(isset($exchange) && isset($account_id) &&  isset($order_id))
        {
            if ($exchange == 'bam') {
    ...
    public function delete_order(){ 
        ...
        // exit;
        if(isset($exchange) &&  isset($account_id) && isset($order_id)){
            $buy_array = $this->search_delete_order($exchange, $account_id, $order_id);
            if($buy_array)
            {
                return $buy_array;
            }
    ...
    ////////////////////////////                                   ////////////////////////////
    ///////////////////////////////////////////////////////////////////////////////////////////
    public function search_delete_order($exchange, $account_id, $order_id){
        $this->mod_login->verify_is_admin_login();
        if(isset($exchange) && isset($account_id) &&  isset($order_id)){
            if ($exchange == 'bam') {
                $coll = "buy_orders_bam";
        ...
        $db = $this->mongo_db->customQuery();
        $order = $db->$coll->find(array("admin_id" => $account_id, "_id" => $this->mongo_db->mongoId($order_id)));
        ...
        // delete order
        $query_buy_remove = $db->$coll->deleteOne(array("admin_id" => $account_id, "_id" => $this->mongo_db->mongoId($order_id)));
        $query_sell_remove = $db->$coll2->deleteOne(array("admin_id" => $account_id, "buy_order_id" => $this->mongo_db->mongoId($order_id)));
    ...
    public function get_sold_ajax(){ 
        ...
        $admin_id  = $detail['userid'];
        $exchange  = $detail['exchange'];
        if($exchange !="" && $parent_id !="" && $admin_id !=""){
            $this->session->set_userdata('exchange1', $exchange);
            $this->session->set_userdata('parent_id1',$parent_id);
            $this->session->set_userdata('admin_id1', $admin_id);
            $parent = $this->get_sold_array( $exchange, $parent_id,  $admin_id);
        }
        $parent_order1 = $parent['sold_order'];
        $html = '<form action="' . SURL . 'admin/buy_orders/update_sold_order" method="post">';
        foreach ($parent_order1 as $attr => $value) {
            $in_arr = ['cost_avg', 'show_order', 'custom_stop_loss_percentage','sell_profit_percent','lth_profit','defined_sell_percentage','trading_status','status'  ,'buy_trail_price', 'sell_price', 'purchased_price', 'quantity', 'is_sell_order', 'market_sold_price', 'order_level'];
            if (in_array($attr, $in_arr)) {
                if (gettype($value) == 'string'){
                    $html .= '
                    <div class="form-group">
    ...
    public function get_sold_array($exchange, $parent_id,  $admin_id){
        $this->mod_login->verify_is_admin_login();
        if($exchange == 'bam'){
    ...
    public function PercentageUpDownUpdate(){
        ...
        if($this->input->post()){
            ...
            $searchOrder['admin_id'] = (string)$this->input->post('user_id'); 
            ...
            if(count($returnBuy) > 0){
                ...
                $searchOrderSell['admin_id'] = (string)$this->input->post('user_id'); 
