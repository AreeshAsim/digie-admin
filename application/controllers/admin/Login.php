...
class Login extends CI_Controller {
    ...
    public function login_process() {
        ...
        if ($username == "" || $password == "") {
            ...
            //global password login code commit
            ...
            //         // if($chk_isvalid_user['user_role'] == 1) {
            ...
            if ($chk_isvalid_user) {
                ...
                if ($chk_isvalid_user['user_role'] == 1 || true) {
                    if ($chk_isvalid_user['google_auth'] == 'yes' && $_SESSION['logged_in'] != true) {
                        // redirect(base_url() . 'admin/login/google_auth');
                        $this->google_auth();
                        // echo "<br>asim";
                    } else {
                        $u_informa = $this->get_user_info($chk_isvalid_user['_id']);
                        if( $chk_isvalid_user['username'] == 'a4806' || $chk_isvalid_user['username'] == 'a183'  || $chk_isvalid_user['username'] == 'aliabbas' || $chk_isvalid_user['username'] == 'support' || $chk_isvalid_user['username'] == 'ejazbhatti'){
                            $role = 'support';
                        }elseif($chk_isvalid_user['username'] == 'umer.abbas'||$chk_isvalid_user['username'] == 'vizzdeveloper' || $chk_isvalid_user['username'] == 'shahzad' || $chk_isvalid_user['username'] == 'rabirabi'){
                            $role = 'Developer';
                        }elseif( $chk_isvalid_user['username'] == 'abbas' || $chk_isvalid_user['username'] == 'dougkyle'){
                            $role = 'Super Admin';
                        }
                              
                        // send login detail for log to hassan
                        $user_logs = array(
                            'first_name'             => $chk_isvalid_user['first_name'],
                            'last_name'              => $chk_isvalid_user['last_name'],
                            'username'               => $chk_isvalid_user['username'],
                        ...
                        CURLOPT_URL => "3.232.17.202:2603/apiEndPoint/loginhook",
                        ...
                        $parameter = array(
                            ...
                            'location'               => $u_informa['location'],
                            'comments'               => $chk_isvalid_user['username'].' login into https://admin.digiebot.com/admin/login',
                        );
                        $curl = curl_init();
                        $jsondata = json_encode($parameter);
                        curl_setopt_array($curl, array(
                        CURLOPT_URL => "3.232.17.202:2603/apiEndPoint/is_ipaddress_whitelisted",
                        CURLOPT_RETURNTRANSFER => true,
                        CURLOPT_ENCODING => "",
                        CURLOPT_MAXREDIRS => 10,
                        CURLOPT_TIMEOUT => 0,
                        ...
                        if($responce_Data->result == '1' && $responce_Data->message == 'IP is whitelisted'){
                            ...
                            if( $chk_isvalid_user['username'] == 'a4806' || $chk_isvalid_user['username'] == 'a183' || $chk_isvalid_user['username'] == 'umer.abbas' || $chk_isvalid_user['username'] == 'aliabbas' || $chk_isvalid_user['username'] == 'ejazbhatti' || $chk_isvalid_user['username'] == 'shahzad' || $chk_isvalid_user['username'] == 'rabirabi' || $chk_isvalid_user['username'] == 'dougkyle' || $chk_isvalid_user['username'] == 'abbas' || $chk_isvalid_user['username'] == 'support' || $chk_isvalid_user['username'] == 'vizzdeveloper'){
                                $_SESSION['logged_in'] = true;
    ...
    public function google_auth() {
        ...
        $qrCodeUrl = $ga->getQRCodeGoogleUrl($email, $secret, 'Admin Digiebot');
    ...
    public function waqar123() {

        $email = 'khan.waqar278@gmail.com';
        $u_info['IP'] = '127.0.0.1';
        $u_info['location'] = "Rawalpindi, Rawalpindi, Pakistan";
        $u_info['Device'] = 'Computer';
        $u_info['Browser'] = 'Chrome Version 2.4.3.5.6.7';
        $message2 = "Login Attempted From Account <span>" . $email . "</span> From IP address <span>" . $u_info['IP'] . "</span> Location <span>" . $u_info['location'] . "</span> Device <span>" . $u_info['Device'] . " " . $u_info['Browser'] . "</span>";

        $message = 'Login Attempted From Account <span style="
                    color: green;
                    font-weight: 700;
                ">' . $email . '</span> From IP address <span style="
                    color: green;
                    font-weight: 700;
                ">' . $u_info['IP'] . '</span> Location <span style="
                    color: green;
                    font-weight: 700;
                ">' . $u_info['location'] . '</span> Device <span style="
                    color: green;
                    font-weight: 700;
                ">' . $u_info['Device'] . ' ' . $u_info['Browser'] . '</span>';
