...
//             $disp_arr_buy_mapped = array();
//             $disp_arr_buy_unmapped = array();
//             $disp_arr_sell_mapped = array();
//             $disp_arr_sell_unmapped = array();
//             $disp_arr = array();
//             $index = 1;
...
//                         $new_obj_['url'] = '';
//                         array_push($disp_arr_sell_unmapped, $new_obj_);
//                     }
//                 } //!$type
//                 else {
//                     $new_obj = array();
...
//                         array_push($disp_arr_buy_mapped, $new_obj);
//                     } //$search != ''
//                     else {
//                         $new_obj['id'] = '';
//                         $new_obj['bid'] = $binance_id;
...
//                         $new_obj['url'] = '';
//                         array_push($disp_arr_buy_unmapped, $new_obj);
//                     }
//                 }
//                 $disp_arr[] = $new_obj;
//             } //$order_history as $obj
//             $buy_binance_id_arr = array_unique(array_column($disp_arr_buy_mapped, 'bid'));
//             $sell_binance_id_arr = array_unique(array_column($disp_arr_sell_mapped, 'bid'));
//             $full_sell = array();
//             foreach ($sell_binance_id_arr as $binance_id) {
//                 $sell_arr = $this->search($disp_arr_sell_mapped, "bid", $binance_id);
//                 array_push($full_sell, $sell_arr);
//             }
//             foreach ($buy_binance_id_arr as $binance_id) {
//                 $id_arr = array();
//                 $buy_arr = $this->search($disp_arr_buy_mapped, "bid", $binance_id);
//             $cid =   $buy_arr[0]['cID'];
//                 foreach ($full_sell as $one) {
//                     if ($cid == $one[0]['ID']) {
//                         $newOne = $one;
//                         break;
//                     }
...
//             ///  print_r($full_arr_test);
//             //echo "Unmapped Buy Trades are ". count($disp_arr_buy_unmapped). " And Sell Trades are ". count($disp_arr_sell_unmapped);
            
//             array_multisort(array_column($disp_arr_buy_unmapped, "btime"), SORT_ASC, $disp_arr_buy_unmapped);
//             array_multisort(array_column($disp_arr_sell_unmapped, "btime"), SORT_ASC, $disp_arr_sell_unmapped);
//             $final_buy_unmapped = $this->group_by("bid", $disp_arr_buy_unmapped);
//             $final_sell_unmapped = $this->group_by("bid", $disp_arr_sell_unmapped);

//             // print_r($final_buy_unmapped);
//             // print_r($final_sell_unmapped);

//             foreach($final_buy_unmapped as $key => $value){
//                 $binance_id = $key;
//                 $quantity = array_sum(array_column($value, "bqty"));
//                 $final_buy_unmapped[$key]['total_qty'] = $quantity;
//             }
//             foreach($final_sell_unmapped as $key => $value){
//                 $binance_id = $key;
//                 $quantity = array_sum(array_column($value, "bqty"));
//                 $final_sell_unmapped[$key]['total_qty'] = $quantity;
//             }

//             $final_unmapped_array = array_merge_recursive($final_buy_unmapped, $final_sell_unmapped);


//             $sortkeys = array();
//             foreach ($final_unmapped_array as $row) {
//             $sortkeys[] = $row[0]['btime'];
//             }

//             // sort $data according to $sortkeys
//             array_multisort($sortkeys, $final_unmapped_array);
//             array_multisort(array_column($final_unmapped_array, "btime"), SORT_ASC, $final_unmapped_array);

            
//             $data['mapped'] = $full_arr_test;
//             $data['unmapped'] = $final_unmapped_array;
//         }
//         $coins = $this->mod_coins->get_all_coins();
//         $data['coins'] = $coins;
//         $this->stencil->paint('admin/reports/mapping_report', $data);
//     }
...
//     public function coin_report_waqar() {
//         $this->mod_login->verify_is_admin_login();
//         if ($this->input->post()) {
//             $data_arr['filter_order_data'] = $this->input->post();
//             $this->session->set_userdata($data_arr);
...
//     ///////////////////////////////////////////////////////////////////////////
    ...
    public function coin_report(){ //Asim
        $this->mod_login->verify_is_admin_login();
        $this->load->library('mongo_db_3');
        $db_3 = $this->mongo_db_3->customQuery();
        $coin_array_all = $this->mod_coins->get_all_coins(); 
        $coin = array_column($coin_array_all, 'symbol');

        $price_search['coin']['$in'] = $coin;
        ...
        foreach($final_prices as $price){ 
            if($price['coin'] == 'ETHBTC'){    
                $ethbtc = (float)$price['price']; 
            }elseif($price['coin'] == 'BTCUSDT'){
                $btcusdt = (float)$price['price'];
            }elseif($price['coin'] == 'XRPBTC'){
                $xrpbtc = (float)$price['price'];
            }elseif($price['coin'] == 'XRPUSDT'){
                $xrpusdt = (float)$price['price'];
            }elseif($price['coin'] == 'NEOBTC'){
                $neobtc = (float)$price['price'];
            }elseif($price['coin'] == 'NEOUSDT'){
                $neousdt = (float)$price['price'];
            }elseif($price['coin'] == 'QTUMBTC'){
                $qtumbtc = (float)$price['price'];
            }elseif($price['coin'] == 'QTUMUSDT'){
                $qtumusdt = (float)$price['price'];
            }elseif($price['coin'] == 'XLMBTC'){
                $xmlbtc = (float)$price['price'];
            }elseif($price['coin'] == 'XEMBTC'){
                $xembtc = (float)$price['price'];
            }elseif($price['coin'] == 'POEBTC'){
                $poebtc = (float)$price['price'];
            }elseif($price['coin'] == 'TRXBTC'){
                $trxbtc = (float)$price['price'];
            }elseif($price['coin'] == 'ZENBTC'){
                $zenbtc = (float)$price['price'];
            }elseif($price['coin'] == 'ETCBTC'){
                $etcbtc = (float)$price['price'];
            }elseif($price['coin'] =='EOSBTC'){
                $eosbtc = (float)$price['price'];
            }elseif($price['coin'] =='LINKBTC'){
                $linkbtc = (float)$price['price'];
            }elseif($price['coin'] =='DASHBTC'){
                $dashbtc = (float)$price['price'];
            }elseif($price['coin'] =='XMRBTC'){
                $xmrbtc = (float)$price['price'];
            }elseif($price['coin'] =='ADABTC'){
                $adabtc = (float)$price['price'];
            }elseif($price['coin'] =='LTCUSDT'){
                $ltcusdt = (float)$price['price'];
            }elseif($price['coin'] =='EOSUSDT'){
        ...
        if ($this->input->post()){
            $data_arr['filter_order_data'] = $this->input->post();
            $this->session->set_userdata($data_arr);
            if(!empty($this->input->post('filter_by_search'))){
                if($this->input->post('filter_by_search') == 'both'){ 
                    if($this->input->post('filter_by_level')){
                        $levels = $this->input->post('filter_by_level');
                    }else{
                        $levels = ['level_5', 'level_7', 'level_8', 'level_11', 'level_12', 'level_17', 'level_18'];
                    }  
                    $filter = $this->input->post('filter_by_coin');
                }elseif($this->input->post('filter_by_search') == 'level'){
                    $levels = $this->input->post('filter_by_level');
                    $filter = $this->input->post('filter_by_level');
                }elseif($this->input->post('filter_by_search') == 'coin'){
                    $filter = $this->input->post('filter_by_coin');
                }elseif($this->input->post('filter_by_search') == 'levelWise'){
                    $filter = $this->input->post('filter_by_coin');
                    if($this->input->post('filter_by_level')){
                        $levels = $this->input->post('filter_by_level');
                    }else{
                        $levels = ['level_1','level_2','level_3','level_4','level_5','level_6', 'level_7', 'level_8', 'level_9','level_10', 'level_11', 'level_12', 'level_13','level_14','level_16','level_17', 'level_18', 'level_19','level_20',];
                    }
                }
            }//end outer check
            $collection_name  = $this->input->post('filter_by_mode') == 'live'? 'opportunity_logs_'.$this->input->post('exchange') : 'opportunity_logs_test_'.$this->input->post('exchange');

            if($_POST['filter_by_start_date'] != "" && $_POST['filter_by_end_date'] != "") {
                $start_time = $this->mongo_db->converToMongodttime(date('Y-m-d 00:00:00', strtotime($_POST['filter_by_start_date'])));
               
                $end_time = $this->mongo_db->converToMongodttime(date('Y-m-d 23:59:59', strtotime($_POST['filter_by_end_date'])));
                $search['created_date'] = array('$gte' => $start_time, '$lte' => $end_time);
                $search_level['created_date'] = array('$gte' => $start_time, '$lte' => $end_time);

                $start_date = date('Y-m-d 00:00:00', strtotime($_POST['filter_by_start_date']));
                $end_date   = date('Y-m-d 23:59:59', strtotime($_POST['filter_by_end_date']));
            }else{
                $start  = date('Y-m-d 00:00:00' , strtotime('-1 month'));
                $start_time = $this->mongo_db->converToMongodttime($start);

                $end_time = $this->mongo_db->converToMongodttime(date('Y-m-d 23:59:59'));
                $search['created_date'] = array('$gte' => $start_time, '$lte' => $end_time);
                $search_level['created_date'] = array('$gte' => $start_time, '$lte' => $end_time);
            ...
            if( $this->input->post('filter_by_search') == 'level' || $this->input->post('filter_by_search') == 'coin' || $this->input->post('filter_by_search') == 'both'){
                foreach($filter as $filterValue){
                    ...
                    $ten_min_value = 0;
                    if($this->input->post('filter_by_search') == 'both'){
                        $search['coin'] = $filterValue;
                        $search['level']['$in'] = $levels;
                    }else{
                        $search[$this->input->post('filter_by_search')] = $filterValue;
                    }
                    $data_return = $db->$collection_name->find($search);
                    $response = iterator_to_array($data_return);
                    // 5 count variable
                    $countOneLessPerMax = 0; 
                    $countOnePerMax = 0;
                    $countOneHalfPerMax = 0;
                    $countTwoPerMax = 0;
                    $countTwoHalfPerMax = 0;
                    $countThreePerMax = 0;
                    $countThreeHalfPerMax = 0;
                    $countFourPerMax = 0;
                    $countFourHalfPerMax = 0;
                    // 10 count variable
                    $tenCountOneLessPerMax = 0;
                    $tenCountOnePerMax = 0;
                    $tenCountOneHalfPerMax = 0;
                    $tenCountTwoPerMax = 0;
                    $tenCountTwoHalfPerMax = 0;
                    $tenCountThreePerMax = 0;
                    $tenCountThreeHalfPerMax = 0;
                    $tenCountFourPerMax = 0;
                    $tenCountFourHalfPerMax = 0;

                    if(count($response) > 0){
                        foreach($response as $value){
                            if(isset($value['oppertunity_missed'])){
                                $oppsMissedcount++;
                            }
                            if(!isset($value['oppertunity_missed']) && $value['avg_sold'] == 0){
                                $soldAvgZeroCount++;
                            }
                            if( $value['avg_open_lth'] == 0 && !isset($value['oppertunity_missed'])){
                                $openLthAvgZeroCount++;
                            }
                            $sold_sum += $value['sold'];
                            $otherStatus += $value['other_status'];
                            $avg_sum += $value['avg_sold'];
                            $per_trade_sold += $value['avg_sold'] * $value['sold'];
                            $open_lth += $value['open_lth'];
                            $avg_open_sum +=$value['avg_open_lth'];
                            $array_output = [
                            ...
                            $max_cal_2 = 0;
                            $per_5_hour_max = 0;
                            if(isset($value['5_max_value']) && $value['5_max_value']>0 ){
                                ...
                                $five_min_value +=  $per_5_hour_min;

                                $min_cal_1 = (float)$value['10_max_value'] - $value['purchase_price'];
                                $min_cal_2 = ($value['10_max_value'] + $value['purchase_price']) / 2;
                                $per_10_hour_max =  ($min_cal_1/ $min_cal_2)*100;
                                $ten_max_value += $per_10_hour_max;

                                $min_cal_1 = (float)$value['10_min_value'] - $value['purchase_price'];
                                $min_cal_2 = ($value['10_min_value'] + $value['purchase_price']) / 2;
                                $per_10_hour_min =  ($min_cal_1/ $min_cal_2)*100;
                                $ten_min_value += $per_10_hour_min;

                                
                                //5 hours
                                if($per_5_hour_max < 1){
                                    $countOneLessPerMax += 1; 
                                }
                                if($per_5_hour_max >= 1){
                                    $countOnePerMax += 1;
                                }
                                if($per_5_hour_max >= 1.5){
                                    $countOneHalfPerMax += 1;
                                }
                                if($per_5_hour_max >= 2){
                                    $countTwoPerMax += 1; 
                                }
                                if($per_5_hour_max >= 2.5 ){
                                    $countTwoHalfPerMax += 1;
                                }
                                if($per_5_hour_max >= 3 ){
                                    $countThreePerMax += 1;
                                }
                                ...
                                //10 hour
                                if($per_10_hour_max < 1){
                                    $tenCountOneLessPerMax += 1; 
                                }
                                if($per_10_hour_max >= 1){
                                    $tenCountOnePerMax += 1;
                                }
                                if($per_10_hour_max >= 1.5){
                                    $tenCountOneHalfPerMax += 1;
                                }
                                if($per_10_hour_max >= 2){
                                    $tenCountTwoPerMax += 1; 
                                }
                                if($per_10_hour_max >= 2.5 ){
                                    $tenCountTwoHalfPerMax += 1;
                                }
                                if($per_10_hour_max >= 3 ){
                                    $tenCountThreePerMax += 1;
                                }
                            
                            }//end if if(isset($value['5_max_value']) && $value['5_max_value']>0 ){

                                
                        }//end foreach

                        $completion_total_new = $open_lth + $otherStatus +  $sold_sum;
                        if($sold_sum == 0){
                        $compeletion_bar = 0;
                        }elseif( ($open_lth + $otherStatus ) == 0){
                        $compeletion_bar = 100;
                        }else{
                        $compeletion_bar = ($sold_sum / $completion_total_new )*100;
                        }
                        $array_output['completionBar'] = $compeletion_bar;
                        if($this->input->post('filter_by_search') == 'coin' || $this->input->post('filter_by_search') == 'both'){
                            $searchOpportunity['coin']  = $filterValue;
                            $coin = $value['coin'];
                        }elseif($this->input->post('filter_by_search') == 'level'){
                            $searchOpportunity['coin']  = $value['coin'];
                            $coin = $value['coin'];
                        }
                        $params = [   
                            'coin'       => $coin,
                        ...
                        $maxCal = (float)$api_response->data->max_price - $value['purchase_price'];
                        $maxCal2 = ($api_response->data->max_price + $value['purchase_price']) / 2;
                        $maxOverAll =  ( $maxCal/ $maxCal2)*100;
                        $array_output['maxOverAll'] = $maxOverAll;
                        
                        $minCal = (float)$api_response->data->min_price - $value['purchase_price'];
                        $minCal2 = ($api_response->data->min_price + $value['purchase_price']) / 2;
                        $minOverAll =  ($minCal/ $minCal2)*100;
                        $array_output['minOverAll'] = $minOverAll;                 

                        //starting to ending status
                        
                        $lookUpMax = [
                            [
                                '$match' => [
                                    'timestampDate' => ['$lte' => $end_time],
                                    'coin' => $coin
                                ],
                            ],
                            [
                        ...
                        $endPrice = $db_3->market_chart->aggregate($lookUpMax);
                        $endPriceResponse = iterator_to_array($endPrice);

                        //current status
                        $lookUp = [
                            [
                                '$match' => [
                                    'timestampDate' => ['$eq' => $start_time],
                                    'coin' => $coin
                                ],
                            ],
                            [
                        ...
                        $strtPrice = $db_3->market_chart->aggregate($lookUp);
                        $strtPriceResponse = iterator_to_array($strtPrice);

                        $difference =  $endPriceResponse[0]['close_price'] - $strtPriceResponse[0]['min_price'];
                        $percentage =  ($difference / $strtPriceResponse[0]['min_price'])*100;
                        $array_output['status'] = $percentage;
                        if($coin == 'ADABTC'){
                            $val1 = $adabtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($adabtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'LTCUSDT'){
                            $val1 = $ltcusdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($ltcusdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'EOSUSDT'){
                            $val1 = $eosusdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($eosusdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'EOSBTC'){
                            $val1 = $eosbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($eosbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'LINKBTC'){
                            $val1 = $linkbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($linkbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'DASHBTC'){
                            $val1 = $dashbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($dashbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'XMRBTC'){
                            $val1 = $xmrbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($xmrbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'ETHBTC'){
                            $val1 = $ethbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($ethbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'ZENBTC'){
                            $val1 = $zenbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($zenbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'ETCBTC'){
                            $val1 = $etcbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($etcbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'TRXBTC'){
                            $val1 = $trxbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($trxbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'BTCUSDT'){
                            $val1 = $btcusdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($btcusdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'XRPBTC'){
                            $val1 = $xrpbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($xrpbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'XRPUSDT'){
                            $val1 = $xrpusdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($xrpusdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'NEOBTC'){
                            $val1 = $neobtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($neobtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'NEOUSDT'){
                            $val1 = $neousdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($neousdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'QTUMBTC'){
                            $val1 = $qtumbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($qtumbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'XLMBTC'){
                            $val1 = $xmlbtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($xmlbtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'QTUMUSDT'){
                            $val1 = $qtumusdt - $strtPriceResponse[0]['min_price'];
                            $val2 = ($qtumusdt + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'XEMBTC'){
                            $val1 = $xembtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($xembtc + $strtPriceResponse[0]['min_price'])/2;
                        }elseif($coin == 'POEBTC'){
                            $val1 = $poebtc - $strtPriceResponse[0]['min_price'];
                            $val2 = ($poebtc + $strtPriceResponse[0]['min_price'])/2;
                        }
                        $percentage  = ($val1/ $val2)*100;
                        $array_output['currentAvg'] = $percentage;
                        $array_output['countOpportunity'] = count($response);

                        unset($api_response);
                            // 5 hours
                        if($countOneLessPerMax > 0){
                            $array_output['countOneLessPerMax'] = $countOneLessPerMax;
                            $array_output['perOneLessPerMax'] = (($countOneLessPerMax / count($response))*100);
                        }
                        if($countOnePerMax > 0){
                            $array_output['countOnePerMax'] = $countOnePerMax;
                            $array_output['perOnePerMax'] = (($countOnePerMax / count($response))*100);
                        }
                        if($countOneHalfPerMax > 0){
                            $array_output['countOneHalfPerMax'] = $countOneHalfPerMax;
                            $array_output['perOneHalfPerMax'] = (($countOneHalfPerMax / count($response))*100);
                        }
                        if($countTwoPerMax > 0){
                        
                            $array_output['countTwoPerMax'] = $countTwoPerMax;
                            $array_output['perTwoPerMax'] = (($countTwoPerMax / count($response))*100);
                        }
                        if($countTwoHalfPerMax > 0){
                            $array_output['countTwoHalfPerMax'] = $countTwoHalfPerMax;
                            $array_output['perTwoHalfPerMax'] = ($countTwoHalfPerMax / count($response))*100;
                        }
                    
                        if($countThreePerMax > 0){
                            $array_output['countThreePerMax'] = $countThreePerMax;
                            $array_output['perThreePerMax'] = ($countThreePerMax / count($response))*100;
                        }
                        if($countThreeHalfPerMax > 0){
                            $array_output['countThreeHalfPerMax'] = $countThreeHalfPerMax;
                            $array_output['perThreeHalfPerMax'] = ($countThreeHalfPerMax / count($response))*100;
                        }
                        if($countFourPerMax > 0){
                            $array_output['countFourPerMax'] = $countFourPerMax;
                            $array_output['perFourPerMax'] = ($countFourPerMax / count($response))*100;
                        }
                        if($countFourHalfPerMax > 0){
                            $array_output['countFourHalfPerMax'] = $countFourHalfPerMax;
                            $array_output['perFourHalfPerMax'] = ($countFourHalfPerMax / count($response))*100;
                        }
                        
                        //ten hours 
                        if($tenCountOneLessPerMax > 0){
                            $array_output['tenCountOneLessPerMax'] = $tenCountOneLessPerMax;
                            $array_output['tenPerOneLessPerMax'] = ($tenCountOneLessPerMax / count($response))*100;
                        }
                        if($tenCountOneHalfPerMax > 0){
                            $array_output['tenCountOneHalfPerMax'] = $tenCountOneHalfPerMax;
                            $array_output['tenPerOneHalfPerMax']   = ($tenCountOneHalfPerMax / count($response))*100;
                        }
                        if($tenCountTwoPerMax > 0){
                            $array_output['tenCountTwoPerMax'] = $tenCountTwoPerMax;
                            $array_output['tenPerTwoPerMax']   = ($tenCountTwoPerMax / count($response))*100;
                        }
                        if($tenCountTwoHalfPerMax > 0){
                            $array_output['tenCountTwoHalfPerMax'] = $tenCountTwoHalfPerMax;
                            $array_output['tenPerTwoHalfPerMax']   = ($tenCountTwoHalfPerMax / count($response))*100;
                        }
                        if($tenCountThreePerMax > 0){
                            $array_output['tenCountThreePerMax'] = $tenCountThreePerMax;
                            $array_output['tenPerThreePerMax']   = ($tenCountThreePerMax / count($response))*100;
                        }
                        if($tenCountThreeHalfPerMax > 0){
                            $array_output['tenCountThreeHalfPerMax'] = $tenCountThreeHalfPerMax;
                            $array_output['tenPerThreeHalfPerMax']   = ($tenCountThreeHalfPerMax / count($response))*100;
                        }
                        if($tenCountFourPerMax > 0){
                            $array_output['tenCountFourPerMax'] = $tenCountFourPerMax;
                            $array_output['tenPerFourPerMax']   = ($tenCountFourPerMax / count($response))*100;
                        }
                        if($tenCountFourHalfPerMax > 0){
                            $array_output['tenCountFourHalfPerMax'] = $tenCountFourHalfPerMax;
                            $array_output['tenPerFourHalfPerMax']   = ($tenCountFourHalfPerMax / count($response))*100;
                        }
                        

                        $array_output['5_max_value']  =  $five_max_value / count($response);
                        $array_output['5_min_value']  =  $five_min_value / count($response);
                        $array_output['10_max_value'] =  $ten_max_value  / count($response);
                        $array_output['10_min_value'] =  $ten_min_value  / count($response);

                        $array_output['other_status'] = $otherStatus;
                        $array_output['sold_orders']  = $sold_sum;   
                        $array_output['open_lth']     = $open_lth;
                        $array_output['avg_open_lth'] = $avg_open_sum / (count($response) - $oppsMissedcount- $openLthAvgZeroCount);
                        
                        $array_output['avg_sold']     = $avg_sum / (count($response) - $oppsMissedcount- $soldAvgZeroCount);
                        $array_output['per_trade_sold'] = $per_trade_sold / $sold_sum;
                        $data['full_arry'][] = $array_output;
                    }//end if
                }  //end foreach for coins
                 ...
                 foreach($filter as $filterValue){
                    ...
                    foreach($levels as $levl){
                        ...
                        $tenCountFourHalfPerMax = 0;
                        if(count($response) > 0){
                            foreach($response as $calulationByLevel){
                                if(isset($calulationByLevel['oppertunity_missed'])){
                                    $oppsMissedcount++;
                                }
                                if(!isset($calulationByLevel['oppertunity_missed']) && $calulationByLevel['avg_sold'] == 0){
                                    $soldAvgZeroCount++;
                                }
                                if( $calulationByLevel['avg_open_lth'] == 0 && !isset($calulationByLevel['oppertunity_missed'])){
                                    $openLthAvgZeroCount++;
                                }
                                $sold_sum += $calulationByLevel['sold'];
                                // echo "<br>solddddddd:  ".$calulationByLevel['sold']."   level0000: ".$levl;
                                $otherStatus += $calulationByLevel['other_status'];
                                $avg_sum += $calulationByLevel['avg_sold'];
                                $per_trade_sold += $calulationByLevel['avg_sold'] * $calulationByLevel['sold'];
                                $open_lth += $calulationByLevel['open_lth'];
                                $avg_open_sum +=$calulationByLevel['avg_open_lth'];
                                ...
                                $max_cal_2 = 0;
                                $per_5_hour_max = 0;
                                if(isset($calulationByLevel['5_max_value']) && $calulationByLevel['5_max_value']>0 ){
                                    $max_cal_1 = (float)$calulationByLevel['5_max_value'] - $calulationByLevel['purchase_price'];
                                    $max_cal_2 = ($calulationByLevel['5_max_value'] + $calulationByLevel['purchase_price']) / 2;
                                    $per_5_hour_max =  ($max_cal_1/ $max_cal_2)*100;
                                    $five_max_value += $per_5_hour_max;

                                    $min_cal_1 = (float)$calulationByLevel['5_min_value'] - $calulationByLevel['purchase_price'];
                                    $min_cal_2 = ($calulationByLevel['5_min_value'] + $calulationByLevel['purchase_price']) / 2;
                                    $per_5_hour_min =  ($min_cal_1/ $min_cal_2)*100;
                                    $five_min_value +=  $per_5_hour_min;

                                    $min_cal_1 = (float)$calulationByLevel['10_max_value'] - $calulationByLevel['purchase_price'];
                                    $min_cal_2 = ($calulationByLevel['10_max_value'] + $calulationByLevel['purchase_price']) / 2;
                                    $per_10_hour_max =  ($min_cal_1/ $min_cal_2)*100;
                                    $ten_max_value += $per_10_hour_max;

                                    $min_cal_1 = (float)$calulationByLevel['10_min_value'] - $calulationByLevel['purchase_price'];
                                    $min_cal_2 = ($calulationByLevel['10_min_value'] + $calulationByLevel['purchase_price']) / 2;
                                    $per_10_hour_min =  ($min_cal_1/ $min_cal_2)*100;
                                    $ten_min_value += $per_10_hour_min;
                                ...
                                }//end if if(isset($value['5_max_value']) && $value['5_max_value
                            }
                            ...
                            }elseif($this->input->post('filter_by_search') == 'level' || $this->input->post('filter_by_search') == 'levelWise'){
                                $searchOpportunity['coin']  = $response[0]['coin'];
                                $coin = $response[0]['coin'];
                            }
                            $params = [   
                                'coin'       => $coin,
                            ...
                            $maxCal = (float)$api_response->data->max_price - $response[0]['purchase_price'];
                            $maxCal2 = ($api_response->data->max_price + $response[0]['purchase_price']) / 2;
                            $maxOverAll =  ( $maxCal/ $maxCal2)*100;
                            $array_output['maxOverAll'] = $maxOverAll;
                            
                            $minCal = (float)$api_response->data->min_price - $response[0]['purchase_price'];
                            $minCal2 = ($api_response->data->min_price + $response[0]['purchase_price']) / 2;
                            $minOverAll =  ($minCal/ $minCal2)*100;
                            $array_output['minOverAll'] = $minOverAll;                 
                            ...
                            $array_output['level'] = $levl;
                            $data['full_arry'][] = $array_output;
                        }
                    }//end response count > 0 check
                }  //end foreach for coins
            }
        }
...
//             $search_sting = trim($this->input->post('filter_by_name'));
//             $search_sting = str_replace("\\", "\\\\", $search_sting);
//             $search_sting_like = new MongoDB\BSON\Regex(".*{$search_sting}.*", 'i');

//             $search_arr['settings.title_to_filter'] = $search_sting_like;
//         }
...
//         } else {
//             $session_user_id = $this->session->userdata('admin_id');
//             $search_arr['settings.admin_id'] = (string) $session_user_id;
...
//         $data['options'] = $option_arr;
//         $data['setting'] = $iter;
//         $data['filter_user_data1'] = $this->input->post();
//         $coins = $this->mod_coins->get_all_coins();
//         $data['coins'] = $coins;
...
//         $get1 = $this->mongo_db->get("users");
//         $results1 = iterator_to_array($get1);
//         for($i = 0; $i < count($results1); $i++){
//             $user_id = $results1[$i]['_id'];
//             $final_class = "";
//             $this->mongo_db->where(array("user_id" => ($user_id)));
//             $get = $this->mongo_db->get("user_wallet_comaprison");
//             $results = iterator_to_array($get);
//             $balance_arr = $results[0]["balances"];
           
//             $response = '<table class="table table-dark">
//             <thead>
//               <tr>
//                 <th scope="col">Coin</th>
//                 <th scope="col">Binance Balance</th>
//                 <th scope="col">Digie Balance</th>
...
//                 $binance_balance = $value['binance_balance'];
//                 $wallet_balance = $value['wallet_balance'];
//                 if($open_balance > $binance_balance || $open_balance > $wallet_balance){
//                     $class = "danger";
//                     $final_class = $class." blink_me";
//                 }else{
//                     $class = "default";
//                 }
//                 $response.= '<tr class="'.$class.'">
//                 <td scope="col">'.$key.'</td>
//                 <td scope="col">'.$value['binance_balance'].'</td>
//                 <td scope="col">'.$value['wallet_balance'].'</td>
//                 <td scope="col">'.$value['open_balance'].'</td>
//               </tr>';
//             }
//             $response.='</tbody>
//           </table>';

//             $results1[$i]['html'] = $response;
//             $results1[$i]['class'] = $final_class;
//         }
//         $data['users'] = $results1;

        
//         $this->stencil->paint('admin/reports/user_wallet_comaprison', $data);
//     }

//     public function user_wallet_report_ajax(){
//         $id = $this->input->post("id");
//         $this->mongo_db->where(array("user_id" => $this->mongo_db->mongoID($id)));
//         $get = $this->mongo_db->get("user_wallet_comaprison");
//         $results = iterator_to_array($get);
//         $balance_arr = $results[0]["balances"];
...
//     }
    ...
    // public function run_market_trending($symbol) {
    //     $array['volume_increasing'] = 2;
    //     $array['lh_tw_contracts'] = array(10, 5, 3, 2);
    //     $array['lh_lw_contracts'] = array(10, 5, 3, 2);
    //     $array['lh_lwb_contracts'] = array(10, 5, 3, 2);
    //     $array['five_min_btc_change'] = 2;
    //     $array['lh_bc_1_qty'] = 2;
    //     $array['lh_bc_2_qty'] = 2;
    //     $array['lh_bc_3_qty'] = 2;
    //     $array['lh_bc_4_qty'] = 2;
    //     $array['lh_bc_5_qty'] = 2;
    //     $array['lh_bc_10_qty'] = 2;
    //     $array['lh_tv_percentile'] = 2;
    //     $array['curr_bc_percentile'] = 2;
    //     $array['curr_bc_percentile_time'] = 2;

    //     $this->mongo_db->where(array("coin" => $symbol));
    //     $this->mongo_db->set($array);
    //     $update = $this->mongo_db->update("market_trending");
    // }
...
///////////////////////////////////////////////////////////////////////////
    ...
    // public function  coin_oppertunity_report() {
