...
class Dashboard extends CI_Controller {
    ...
    public function insertTocken(){
        print_r($this->input->post());
        $currentToken = (string)$this->input->post('currentToken');
        $adminId = (string)$this->input->post('adminId');

        $whereStoreTocken['_id'] = $this->mongo_db->mongoId($adminId);
        $saveTocken = [
            'webToken' => $currentToken
        ];
        $set['$set'] =  $saveTocken;
        $db = $this->mongo_db->customQuery();
        $db->users->updateOne($whereStoreTocken, $set);
        return true;
    }
}
