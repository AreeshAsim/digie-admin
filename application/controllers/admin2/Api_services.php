...
class Api_services extends REST_Controller {
	...
	public function login_process_post() {
		...
		if ($username == "" || $password == "") {
			...
			$chk_isvalid_user = $this->mod_login->validate_credentials($username, $password);
			if ($chk_isvalid_user) {
				//echo $secret; exit;
				...
				if ($chk_isvalid_user['google_auth'] == 'yes') {
					...
					$this->mod_api_services->send_confirm_email($chk_isvalid_user['id'], $chk_isvalid_user['email_address']);
				}
	...
	public function verification_code_post() {
		...
		if ($verification_code_type == 'google_auth') {
			...
			if ($checkResult) {
				...
				$message = array(
					'status' => FALSE,
					'message' => 'Google Verification Failure',
				);
				$this->set_response($message, REST_Controller::HTTP_NOT_FOUND);
			}
	...
	public function get_orders_post() {
		...
		// $data = $post_data;
		// $fp = fopen('waqar_test.txt', 'a') or exit("Unable to open file!");;
		// fwrite($fp, $data);
		// fclose($fp);
	...
	public function add_digie_manual_order() {
	...
	public function add_digie_trigger_order() {
