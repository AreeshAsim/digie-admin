...
class Dashboard extends CI_Controller {
    ...
    public function index() {
        ...
        $this->stencil->paint('admin2/dashboard/dashboard', $data);
            ...
            $response = '<table class="table">
                            <thead>
                                <tr>
									...
									<th class="text-success text-">Total (BTC)</th>
                                </tr>
                            </thead>
                          	<tbody>';
			$cnt=0;				
            ...
            $response2 = '<table class="table table-condensed">
                            <thead>
                                <tr>
                                  ...
                                  <th class="text-danger "><strong>Price(BTC)</strong></td>
                                  <th class="text-danger "><strong>Amount(' . $currncy . ')</strong></td>
                                  <th class="text-danger "><strong>Total(BTC)</strong></td>
                                </tr>
                            </thead>
                          	<tbody>';
		    $cnt2=0;						
        ...
        if (count($market_history_arr) > 0) {
            ...
            $cnt3++;
                ...
                if ($_SERVER['REMOTE_ADDR'] == '58.65.164.72') {
                    $response3 .= '<td class="'. $color.'"> '. $cnt3.'</td>
                                    <td class="'. $color.'">' . number_format($value3['price'], 8, '.', '') . '(' . $value3['counter'] . ')</td>
                                    <td class="'. $color.'">' . number_format($value3['quantity'], 2, '.', '') . '</td>
                                    <td class="'. $color.'">';
                    ...
                    $response3 .= '<td class="'. $color.'"> '. $cnt3.'</td>
                                                    <td class="'. $color.'">' . number_format($value3['price'], 8, '.', '') . '</td>
                                                    <td class="'. $color.'">' . number_format($value3['quantity'], 2, '.', '') . '</td>
                                                    <td class="'. $color.'">';
                    $total_price3 = $value3['price'] * $value3['quantity'];
                    $response3 .= number_format($total_price3, 8, '.', '');
...
// ******** Alikhan Work Goes here *********//
            ...
            $set_url = "http://users.digiebot.com/cronjob/getReportUserExists/?report_id=$report_id&email_address=$email_address";
            $curl = curl_init();
...
//End of  live order check
    ...
    public function autoload_market_statistics() {
        ...
        if ($type == 'sell') {
            $response .= '<li class="list-group-item d-flex justify-content-between">
    ...
    public function autoload_notifcations($id = '') {
