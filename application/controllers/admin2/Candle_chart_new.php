 ...
 *
 */
class Candle_chart_new extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('admin/mod_new_candle_chart');
	...
	public function index() {
		...
		$periods = '15m';
		// Comments BY Ali
		$order_data = $this->mod_new_candle_chart->get_order_array($global_symbol, $admin_id, $global_mode);
		$candel_stick_arr = $this->mod_new_candle_chart->get_candelstick_data_from_database($global_symbol, $periods);
		...
		$task_manager_setting_arr = $this->mod_new_candle_chart->get_task_manager_setting($global_symbol);
		...
		$draw_target_zone_arr = $this->mod_new_candle_chart->get_chart_target_zones($global_symbol);
		...
		if (count($draw_target_zone_arr) > 0) {
			...
			foreach ($draw_target_zone_arr as $target_zone_value) {
				...
				$get_market_history_for_candel = $this->mod_new_candle_chart->get_candle_price_volume_detail($global_symbol, $start_date, $end_date, $unit_value);
		...
		$all_candle_volume_detail = $this->mod_new_candle_chart->get_candle_price_volume_detail($global_symbol, $from_date_for_candel, $end_date_for_candel, $unit_value = 0);

		$all_Hour_candle_volume_detail = $this->mod_new_candle_chart->get_hour_volume_array_detail($global_symbol, $from_date_for_candel, $end_date_for_candel);
		...
		foreach ($candel_stick_arr as $candel_stick_single_arr) {
			...
			$end_date_1 = date("Y-m-d H:i:59", strtotime("+59 minutes", $openTime_current_time_zone / 1000));
			$down_status = $this->mod_new_candle_chart->calculate_pressure_up_and_down($start_date_1, $end_date_1, $candel_stick_single_arr['coin'], $pressure_type = 'down');

				$up_status = $this->mod_new_candle_chart->calculate_pressure_up_and_down($start_date_1, $end_date_1, $candel_stick_single_arr['coin'], $pressure_type = 'up');
				$calculate_up_down_wall = $this->mod_new_candle_chart->calculate_up_down_wall($candel_stick_single_arr['coin'], $start_date_1);
			...
			$formula = 'one';
			$time   = '15';
			$dataArr = $this->mod_new_candel->getSentimentReport($type, $source, $coin, $start, $end, $formula, $time);
            
			/*$candel_stick_single_arr['positive'] =  $dataArr[0]['sum_positive']; 
			$candel_stick_single_arr['negitive'] =  $dataArr[0]['sum_nageative'];
			 */
			 
			$candel_stick_single_arr['down_status'] =  $dataArr[0]['sum_positive'];
			$candel_stick_single_arr['up_status'] =$dataArr[0]['sum_nageative'];
			 
			
			//$data['sentimentArray'] = $dataArr;
			/* echo "<prE>";
					          print_r($dataArr);
					          exit;
							  // By ali sentiment Report Goe END here
		...
		$this->stencil->paint('admin/candle_stick/new_candlesdtick_dyamic_7', $data);
	}
	...
	public function autoload_candle_stick_data_from_database_ajax() {
		...
		$candel_stick_arr = $this->mod_new_candle_chart->get_candelstick_data_from_database($global_symbol, $periods, $from_date_object, $to_date_object, $previous_date, $forward_date);
		...
		foreach ($candel_stick_arr as $candel_stick_single_arr) {
			...
			*/
			...
			$candel_stick_single_arr['down_status'] =  $dataArr[0]['t_pos_divide_t'];
			
			$candel_stick_single_arr['up_status'] =  $dataArr[0]['t_neg_divide_t'];
	...
	public function delete_data_from_data_base() {
		...
		$this->mod_new_candle_chart->delete_data_from_data_base();
	...
	public function save_candel_ajax() {
		...
		echo $update_candels = $this->mod_new_candle_chart->update_candel_for_formul_values($data, $global_symbol);
	...
	public function save_task_manager_setting_ajax() {
		...
		$this->mod_new_candle_chart->save_task_manager_setting($this->input->post(), $global_symbol);
