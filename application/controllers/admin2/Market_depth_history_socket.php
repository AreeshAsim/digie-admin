...
class Market_depth_history_socket extends CI_Controller {
					...
					if (empty($this->old_array[$symbol])) {
						...
						foreach ($this->new_array[$symbol]['bids'] as $key => $value) {
							...
							if ($check_duplicate == 'no') {
								...
								$this->update_data($upd_data, $symbol, $price, 'bid');
						...
						foreach ($this->new_array[$symbol]['asks'] as $key => $value) {
							...
							if ($check_duplicate == 'no') {
								...
								$this->update_data($update_data_2, $symbol, $price, 'ask');
						...
						foreach ($array_bid_change as $key => $value) {
							...
							if ($check_duplicate == 'no') {
								...
								$this->update_data($update_data_3, $symbol, $price, 'bid');
						...
						foreach ($array_ask_change as $key => $value) {
							...
							if ($check_duplicate == 'no') {
								...
								$this->update_data($update_data_4, $symbol, $price, 'ask');
	...
	public function update_data($data, $symbol, $price, $type) {

		$this->mongo_db->where(array('price' => $price, 'coin' => $symbol, 'type' => $type, 'status' => 1));
		...
		$get_arrrr = $this->mongo_db->get('market_depth_history');

	}

	public function market_depth_data_view() {
		// $where['created_date'] = array('$gte' => $this->mongo_db->converToMongodttime(date("Y-m-d H:i:s", "2018-09-08 10:00:00")), '$lte' => $this->mongo_db->converToMongodttime(date("Y-m-d H:i:s", "2018-09-09 10:00:00")));
