  ...
  <div class="widget widget-inverse">
    <div class="widget-body">
      <form method="POST" action="<?php echo SURL; ?>admin/users_list/investment_report">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 ax_8">
          <div class="Input_text_s">
            ...
            <select id="filter_select" name="filter_select[]" multiple="multiple" type="text" class="form-control filter_by_name_margin_bottom_sm filter_select" >
              <option value="limit_exceed_btc"<?php if (in_array('limit_exceed_btc', $filter_data['filter_select'])) {?> selected <?php }?>>Limit Exceed BTC</option>
              <option value="limit_exceed_usdt"<?php if (in_array('limit_exceed_usdt', $filter_data['filter_select'])) {?> selected <?php }?>>Limit Exceed USDT</option>
              <option value="previous_limit_exceed_btc"<?php if (in_array('previous_limit_exceed_btc', $filter_data['filter_select'])) {?> selected <?php }?>>Previous Day Limit Exceed BTC</option>
              <option value="previous_limit_exceed_usdt"<?php if (in_array('previous_limit_exceed_usdt', $filter_data['filter_select'])) {?> selected <?php }?>>Previous Day Limit Exceed USDT</option>
              <option value="join_last_week"<?php if (in_array('join_last_week', $filter_data['filter_select'])) {?> selected <?php }?>>Join Last Week</option>
              <option value="bnb"<?php if (in_array('bnb', $filter_data['filter_select'])) {?> selected <?php }?>>BNB Exists</option>
              <option value="bnbNotExists"<?php if (in_array('bnbNotExists', $filter_data['filter_select'])) {?> selected <?php }?>>BNB Not Exists</option>
              <option value="balance_exists"<?php if (in_array('balance_exists', $filter_data['filter_select'])) {?> selected <?php }?>>Balance Exists</option>
              <option value="api_key"<?php if (in_array('api_key', $filter_data['filter_select'])) {?> selected <?php }?>>Api Key Valid</option>
              <option value="block_user"<?php if (in_array('block_user', $filter_data['filter_select'])) {?> selected <?php }?>>Block User</option>
              <option value="api_key_not"<?php if (in_array('api_key_not', $filter_data['filter_select'])) {?> selected <?php }?>>Api Key Not Valid</option>
              <option value="atg_disable"<?php if (in_array('atg_disable', $filter_data['filter_select'])) {?> selected <?php }?>>ATG Disable</option>
              <option value="atg_enable"<?php if (in_array('atg_enable', $filter_data['filter_select'])) {?> selected <?php }?>>ATG Enable</option>
              <option value="todayZerobtc"<?php if (in_array('todayZerobtc', $filter_data['filter_select'])) {?> selected <?php }?>>Today Buy Zero BTC</option>
              <option value="todayZerousdt"<?php if (in_array('todayZerousdt', $filter_data['filter_select'])) {?> selected <?php }?>>Today Buy Zero USDT</option>
              <option value="predayZerobtc"<?php if (in_array('predayZerobtc', $filter_data['filter_select'])) {?> selected <?php }?>>Previous day Buy Zero BTC</option>
              <option value="predayZerousdt"<?php if (in_array('predayZerousdt', $filter_data['filter_select'])) {?> selected <?php }?>>Previous day Buy Zero USDT</option>
              <option value="pointsRemaningNegitive"<?php if (in_array('pointsRemaningNegitive', $filter_data['filter_select'])) {?> selected <?php }?>>Remaning Points Negitive</option>
              <option value="pointsRemaningPositive"<?php if (in_array('pointsRemaningPositive', $filter_data['filter_select'])) {?> selected <?php }?>>Remaning Points Positive</option>
              <option value="USDTbalance_exists"<?php if (in_array('USDTbalance_exists', $filter_data['filter_select'])) {?> selected <?php }?>>USDT Balance Exists</option>
              <option value="BTCbalance_exists"<?php if (in_array('BTCbalance_exists', $filter_data['filter_select'])) {?> selected <?php }?>>BTC Balance Exists</option>    
              <option value="takingOrderParents"<?php if(in_array('takingOrderParents', $filter_data['filter_select'])){?> selected <?php } ?>>Taking Order Parents Exists</option>  
              <option value="newOrderParents"<?php if(in_array('newOrderParents', $filter_data['filter_select'])){?> selected <?php } ?>>Parent Status New</option> 
              <option value="greaterOne"<?php if(in_array('greaterOne', $filter_data['filter_select'])){?> selected <?php } ?>>Today Buy Both GreaterThan 1 Count</option> 
              <option value="preGreaterOne"<?php if(in_array('preGreaterOne', $filter_data['filter_select'])){?> selected <?php } ?>>Previous Buy Both GreaterThan 1 Count</option> 
        ...
        <div class="col-xs-12 col-sm-12 col-md-1 ax_23">
          <div class="Input_text_s">
            <label></label>   
  ...
  *, *::after, *::before {
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form method="POST" action="<?php echo SURL;?>admin/users_list/addComments">
              <table>
                <tr><td><textarea id="comments" name="comments" rows="4" cols="25" placeholder="Add your reviews"></textarea></td></tr>
                <tr><td><input type="hidden" id="exchange1" name="exchange1" value=""></td></tr>  
                <tr><td><input type="hidden" id="admin_id1" name="admin_id1" value=""></td></tr>
              </table>
              <br>
    ...
    <div class="widget-body padding-bottom-none col-xs-12 col-sm-12 col-md-12 ax_12">
      <table class=" table table-bordered table table-striped" id="datatable" width="100%"> 
        <thead class="theadd">
          ...
          <th><a href="#" data-toggle="tooltip" title="investment % according to Pkg">Last 30 Days Investment %</a></th>
  ...
  foreach ($final_array as $key => $value) {
    ...
    <tr style="text-align:center;">
        ...
        <td><span title="tradeable balance Limit using pakage $: <?php echo $value['tradeAbleBalanceBaseOnPakge']; ?>" style="float:right"><?php echo number_format($value['actual_deposit'], 2); ?><span></td>
        ...
        <td style="text-align:center">
          ...
          //lth    
            ...
            if( $value['exchange_enabled'] == 'yes' && $value['total_balance'] >= 20){
      ...
      <td>
        ...
        <?php if($value['contact_button'] != 'hide'){ ?>
          
          <button type="button" id= "contact"class="click btn btn-success" data-toggle="modal" data-target="#myModal">Contact</button>
        <?php }else{ ?> 

          <span class="label label-info" title="Contact Date"><?php echo $value['contactNowDate']->toDateTime()->format("Y-m-d H:i:s");?></span>
        <?php } ?>
          
        <?php if($value['good_button'] != 'hide') {?>
          <br><br> 
        ...
        <?php }else{ ?>
          <br>   
          <span class="label label-info" title="Good Now Date" ><?php echo $value['goodNowDate']->toDateTime()->format("Y-m-d H:i:s");?></span>
       <?php } ?>

        <?php if($value['comments_button'] != 'hide'){ ?>
          <br><br>
        <button type="button" class="open btn btn-success" data-toggle="modal" data-target="#myModal2">Add Comments</button>
        <?php } else{?>
          <br>   
          <span class="label label-info" title="Comments Date" ><?php echo $value['commentsDate']->toDateTime()->format("Y-m-d H:i:s");?></span>
          <?php } ?>
      </td>
    </tr>
    <?php $count++; }   ?>  
