    ...
    public function trades($symbols,$counter,$callback)
    {
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector('wss://stream.binance.com:9443/ws/'.strtolower($symbol).'@aggTrade')->then(function ($ws) use ($callback, $symbol, $loop, $endpoint,$counter) {
                ...
                $ws->on('open', function ($code = null, $reason = null) use ($symbol, $loop) {
                    ...
                    $msg = "WEB SOCKET ({$symbol}) Has Been Open";
                    mail("contactmoazamali@gmail.com","Socket Open",$msg);
                    $loop->stop();
                });
                $ws->on('close', function ($code = null, $reason = null) use ($symbol, $loop) {
                    ...
                    mail("contactmoazamali@gmail.com","Socket Closed",$msg);
                    $loop->stop();
                });
