...
class API {
    ...
    public function accountStatus() {
        return $this->signedRequest("v3/accountStatus.html");
    }
    ...
    private function signedRequest($url, $params = [], $method = "GET") {
        ...
        if (isset($json['msg'])) {
            return "signedRequest error: {$data}" . PHP_EOL;
        }
        return $json;
    }
    ...
    private function apiRequest($url, $method = "GET") {
        if (empty($this->api_key)) {
            return ('apiRequest error: API Key not set!');
        }
    ...
    private function balanceData($array, $priceData = false) {
        ...
        if (empty($array) || empty($array['balances'])) {
            $msg = "balanceData error: Please make sure your system time is synchronized, or pass the useServerTime option." . PHP_EOL;
            return [];
        }
    ...
    public function trades($symbols, $counter, $callback) {
        if (!is_array($symbols)) {
            $symbols = [$symbols];
        }
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector('wss://stream.binance.com:9443/ws/' . strtolower($symbol) . '@aggTrade')->then(function ($ws) use ($callback, $symbol, $loop, $endpoint, $counter) {
                $ws->on('message', function ($data) use ($ws, $callback, $loop, $endpoint, $counter) {
    ...
    public function trades_bak($symbols, $callback) {
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector('wss://stream.binance.com:16634/ws/' . strtolower($symbol) . '@aggTrade')->then(function ($ws) use ($callback) {
                $ws->on('message', function ($data) use ($ws, $callback) {
                    $json = json_decode($data, true);
                    $symbol = $json['s'];
                    $price = $json['p'];
                ...
                $ws->on('close', function ($code = null, $reason = null) {
                    ...
                    $ch = curl_init();
                    $url = "https://app.digiebot.com/admin/save_curl/save_error_to_db";
                    ...
                    $msg = "WEB SOCKET ({$symbol}) Has Been Closed";
                    mail("khan.waqar278@gmail.com", "Socket Closed", $msg);
                    $loop->stop();
                });
                ...
                $ws->on('error', function ($e) {
                    ...
                    curl_close($ch);
                    $result = json_decode($result, true);
                    $msg = "WEB SOCKET ({$symbol}) Has Been In Error";
                    mail("khan.waqar278@gmail.com", "Socket Closed", $msg);
                    $loop->stop();
                });
            }, function ($e) use ($loop, $symbol) {
    ...
    public function ticker($symbol, $callback) {
        ...
        \Ratchet\Client\connect('wss://stream.binance.com:16634/ws/' . $endpoint)->then(function ($ws) use ($callback, $symbol) {
    ...
    public function chart($symbols, $interval = "30m", $callback) {
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector('wss://stream.binance.com:16634/ws/' . strtolower($symbol) . '@kline_' . $interval)->then(function ($ws) use ($callback) {
                $ws->on('message', function ($data) use ($ws, $callback) {
                    $json = json_decode($data);
                    $chart = $json->k;
                    $symbol = $json->s;
    ...
    public function userData(&$balance_callback, &$execution_callback = false) {
        ...
        \Ratchet\Client\connect('wss://stream.binance.com:16634/ws/' . $listenKey)->then(function ($ws) {
