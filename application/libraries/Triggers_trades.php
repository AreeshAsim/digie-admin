...
class Triggers_trades {
    ...
    public function lock_ture_rules_for_triggers($coin_symbol,$rule_number,$type,$market_price,$log,$order_level,$trigger_type){
		...
		if($resp){
			...
			$res = $this->CI->mongo_db->update('barrier_trigger_true_rules_collection');
		}else{
			$this->CI->mongo_db->insert('barrier_trigger_true_rules_collection', $insert_arr);
		}
		return true;
    ...
    public function is_trigger_rules_already_log($type,$coin_symbol,$rule_no,$order_level){
		...
		$where['order_level'] = $order_level;	
        $limit['limit']=1;
		$rules_object = $db->barrier_trigger_true_rules_collection->find($where,$limit);
    ...
    function grep_rules($pattern,$input,$flags=0,$type)
    {
    ...
    function get_rule($pattern,$input,$flags=0)
    {
        $keys = preg_grep( $pattern, array_keys( $input ), $flags );
        $vals = array();
        foreach ( $keys as $key )
    ...
    public function is_buy_trading_off(){
        $where['status'] = 'on';
        $where['type'] = 'buy_on_of_trading';
        $this->CI->mongo_db->where($where);
