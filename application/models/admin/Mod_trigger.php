...
//Get All Categories
	...
	public function add_new_trigger($data){
		...
		$ins_data = array(
				'trigger_name' => $this->db->escape_str(trim($trigger_name)),
	...
	public function update_cattype($data){
		...
		$upd_data = array(
				'meta_value' => $this->db->escape_str(trim($value)),
				'cat_id' => $this->db->escape_str(trim($cat_id)),
	...
	public function add_cat_type($data){
		...
		$ins_data = array(
			...
			'variable' => $this->db->escape_str(trim($variable)),
			'field_name' => $this->db->escape_str(trim($field_name)),
			'field_value' => $this->db->escape_str(trim($field_value)),
			'cat_id' => $this->db->escape_str(trim($trigger_id)),
			'html_structure' => $this->db->escape_str(trim($html_structure)),
			'seo_url_name' => $this->db->escape_str(trim($verified_seo_url))
		);
		//Insert the record into the database.
		$this->db->dbprefix('cat_fields');
		...
		/*if($field_id!=''){
			...
			foreach($type_value_array as $row){	
				$ins_data = array(
					...
					'cat_type_id' => $this->db->escape_str(trim($last_id)),
					'cat_type_value' => $this->db->escape_str(trim($row['cat_type_value']))
				);
				//Insert the record into the database.
	...
	public function ajax_add_trigger_type_value($data){
		...
		$ins_data = array(
			'cat_type_id' => $this->db->escape_str(trim($cat_type_id)),
			'cat_type_value' => $this->db->escape_str(trim($cat_type_value))
		);
		//Insert the record into the database.
		$this->db->dbprefix('cat_type_value');
		$ins_into_db = $this->db->insert('cat_type_value', $ins_data);
