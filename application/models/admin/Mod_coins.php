...
class mod_coins extends CI_Model {
	...
	public function get_all_coins_kraken() {
		$this->mongo_db->sort(array('_id' => 1));
		$this->mongo_db->where(array('user_id' => 'global'));
		$get_coins = $this->mongo_db->get('coins_kraken');
		$coins_arr = iterator_to_array($get_coins);
		return $coins_arr;
	} //end get_all_coins

	
	public function get_all_coins_bam() {
		$this->mongo_db->sort(array('_id' => 1));
		$this->mongo_db->where(array('user_id' => 'global', 'exchange_type' => "bam"));
		$get_coins = $this->mongo_db->get('coins_bam');
		$coins_arr = iterator_to_array($get_coins);
		return $coins_arr;
	} //end get_all_coins
	...
	public function add_coin($data) {
		...
		$ins_data = array(
			...
			'contract_time' => $contract_time,
			'contract_size' => $contract_size,
			'big_contract_base' => $big_contract_base,
			'big_contract_delta_base' =>$big_contract_delta_base,
			'user_id' => 'global',
			'created_date' => (trim($created_date)),
		);
		if ($logo1 != '') {
		...
		$upsert['upsert'] = true;
		$res = $custom->$collectionName->updateOne($where, ['$set'=> $ins_data], $upsert);
		// $returnId = iterator_to_array($res);
		// $res->getInsertedId();
	...
	public function get_big_contract_delta_base($symbol) {
		$get_arr = $this->get_coin_from_symbol($symbol);
		return $get_arr['big_contract_delta_base'];
	} //end get_big_contract_base()


	public function get_coin_from_symbol($symbol) {
		$this->mongo_db->where(array('symbol' => $symbol,'user_id' => 'global'));
		$get_coin = $this->mongo_db->get('coins');
