	...
	public function save_on_off_trading($data){
			...
			if(!empty($data)){
				...
				foreach($types_arr as $type){
					...
					$is_exist = $this->check_on_of_trading_exist($type);
					$insert_arr = array('type'=>$type,'status'=>$status,'created_date'=>$created_date_obj);
					...
					if($is_exist){
						...
						// %%%%%%%%%%%%%%%%%% update %%%%%%%%%%%%%%%%%%
							...
							$this->mongo_db->update('trading_on_off_collection');
					}else{

						$this->mongo_db->insert('trading_on_off_collection',$insert_arr);
					}
